...
public class TortureScript : ModuleScript
{
    ...
    [SerializeField]
    private KMSelectable _referencePoint;
    [SerializeField]
    ...
    internal bool _isModuleSolved, _isSeedSet, _isNotEnoughTime, _isRalpMode = false;
    ...
    private void GenerateGrid(int initialFinalValue)
    {
        for(int i = 0; i < _grid.Length; i++)
        {
            ...
            _grid[i].GetComponent<Selectable>().SetValues(i, _isRalpMode ? _rnd.Next(7, 17) : _rnd.Next(3, 5), _grid.Length, initialFinalValue, _modulus);
            _grid[i].GetComponent<Selectable>().SetText(initialFinalValue.ToString());
            _grid[i].GetComponent<Selectable>().SetColour(i, false);
    ...
    private IEnumerator SolveAnimation()
    {
        ...
        while (elapsed < 24.3)
        {
            ...
            if(_solveTimings.Any(x => elapsed - x < 0.0175 && elapsed - x > 0))
            {
                ...
                for (int i = 0; i < _grid.Length; i++)
                    _grid[i].GetComponent<Selectable>().SetSolvedColour(i, _rnd.Next(0, 2) == 1); // _grid[i].GetComponent<Selectable>().SetSolvedColour(i, structure[part][i] == '1');
            }
        }
        
        for (int i = 0; i < _grid.Length; i++)
            _grid[i].GetComponent<Selectable>().SetSolvedColour(i, true);
